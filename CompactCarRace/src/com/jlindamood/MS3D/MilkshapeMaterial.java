...
//    public void loadMaterials(BufferedReader file) throws ParsingErrorException {
//    }
...
/*
    private Texture generateTexture(String basePath) throws FileNotFoundException {
        if (texture.length()==0) return null;
        if (!new File(basePath+texture).exists())
            throw new FileNotFoundException("Couldn't find colormap file " + texture);
...
*/
    private Texture generateTexture(URL baseURL) throws FileNotFoundException {
        ...
        if (textureHelper.getTexture()==null) throw new FileNotFoundException("Couldn't load texture: " + texture);
        return textureHelper.getTexture();
        ...
        myAppearance.setTexture(null);
            ...
            if (transparency!=1){
                TransparencyAttributes transAtt=new TransparencyAttributes(
                        TransparencyAttributes.BLENDED,1-transparency);
